...
FreeImage_AllocateT(FREE_IMAGE_TYPE type, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	...
	if (bitmap != NULL) {
		...
		if (bitmap->data != NULL) {
			memset(bitmap->data,0x00 , dib_size);// ÐÞ¸Ä  WHITe  -> 0 @jiayu
			...
			memset(&fih->bkgnd_color, 0x00, sizeof(RGBQUAD));//// ÐÞ¸Ä  WHITe  -> 0 @jiayu

			fih->transparent        = FALSE;
			fih->transparency_count = 0;
			memset(fih->transparent_table, 0x00, 256);// ÐÞ¸Ä  0xff -> 0x0 @jiayu :2016-02-24 10:50:01
...
FreeImage_Clone(FIBITMAP *dib) {
	...
	if (new_dib) {
		...
		memset(dst_iccProfile, WHITe, sizeof(FIICCPROFILE));
...
FreeImage_SetBackgroundColor(FIBITMAP *dib, RGBQUAD *bkcolor) {
	if(dib) {
		...
		if(bkcolor) {
			...
			memset(bkgnd_color, WHITe, sizeof(RGBQUAD));
		}
		return TRUE;
	}
...
FreeImage_FindFirstMetadata(FREE_IMAGE_MDMODEL model, FIBITMAP *dib, FITAG **tag) {
	...
	if(tagmap) {
		...
		if(handle) {
			...
			if(handle->data) {
				memset(handle->data, WHITe, header_size * sizeof(BYTE));
